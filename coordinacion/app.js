document.addEventListener("DOMContentLoaded", () => {
    // Hide loader when page is fully loaded
    window.addEventListener("load", () => {
        const loader = document.getElementById("loader-wrapper");
        if (loader) {
            loader.classList.add("loader-hidden");
        }
    });

    // Mobile menu toggle
    const menuToggle = document.getElementById("menuToggle");
    const mobileNav = document.getElementById("mobileNav");

    if (menuToggle && mobileNav) {
        menuToggle.addEventListener("click", () => {
            menuToggle.classList.toggle("active");
            mobileNav.classList.toggle("open");
        });
    }

    // Datos de los coordinadores (7 coordinadores)
    const coordinadoresData = [
        {
            foto: "../imagenes/coordinadores/coodinador01.webp",
            nombre: "Abril Masis",
            puesto: "Coordinación General",
            socials: {
                linkedin: "https://www.linkedin.com/in/abril-amanda-mas%C3%ADs-rodr%C3%ADguez-7bb67b2b9/",
                instagram: "🐣"
            }
        },
        {
            foto: "../imagenes/coordinadores/coodinador02.webp",
            nombre: "Andrey Figueroa",
            puesto: "Coordinador de Finanzas & Dev",
            socials: {
                linkedin: "https://www.linkedin.com/in/andrey-figueroa-calder%C3%B3n/",
            }
        },
        {
            foto: "../imagenes/coordinadores/coodinador03.webp",
            nombre: "Sara Diaz",
            puesto: "Coordinadora de Mercadeo",
            socials: {
                linkedin: "https://www.linkedin.com/in/sara-d%C3%ADaz-benavides-4a4120362/",
            }
        },
        {
            foto: "../imagenes/coordinadores/coodinador04.webp",
            nombre: "Dennis Rojas",
            puesto: "Coordinador del HackaTob & Dev",
            socials: {
                linkedin: "https://www.linkedin.com/in/dennis-o-rojas-quesada-749056358/",
            }
        },
        {
            foto: "../imagenes/coordinadores/coodinador05.webp",
            nombre: "Paz Bloise",
            puesto: "Coordinadora de Alianzas",
            socials: {
                linkedin: "https://www.linkedin.com/in/pazbloise/",
            }
        },
        {
            foto: "../imagenes/coordinadores/coodinador06.webp",
            nombre: "Anthony Ureña",
            puesto: "...",
            socials: {
                linkedin: "https://www.linkedin.com/in/anthonyuc/",
            }
        },
        {
            foto: "../imagenes/coordinadores/coodinador07.webp",
            nombre: "Densell Calderón",
            puesto: "Encargado de Logistica",
            socials: {
                instagram: "https://www.instagram.com/dencell.06/"
            }
        }
    ];

    const coordinadoresGrid = document.getElementById("coordinadoresGrid");

    // Función para renderizar una tarjeta
    function renderCoordinatorCard(coord, index) {
        return `
            <div class="speaker-simple-card" data-index="${index}">
                <div class="speaker-avatar-wrap">
                    <img src="${coord.foto}" alt="${coord.nombre}" class="speaker-avatar">
                </div>
                <h3 class="speaker-simple-name">${coord.nombre}</h3>
                <p class="speaker-simple-role">${coord.puesto}</p>
            </div>
        `;
    }

    if (coordinadoresGrid) {
        const cardsHTML = coordinadoresData.map((coord, i) => renderCoordinatorCard(coord, i)).join('');
        coordinadoresGrid.innerHTML = cardsHTML;
    }

    // Lógica del Modal
    const modal = document.getElementById("coordModal");
    const modalClose = document.getElementById("coordModalClose");
    const modalImg = document.getElementById("coordModalImg");
    const modalName = document.getElementById("coordModalName");
    const modalRole = document.getElementById("coordModalRole");
    const modalSocials = document.getElementById("coordModalSocials");

    if (coordinadoresGrid && modal) {
        // Abrir modal al hacer click en una tarjeta
        coordinadoresGrid.addEventListener("click", (e) => {
            const card = e.target.closest(".speaker-simple-card");
            if (!card) return;

            const index = card.getAttribute("data-index");
            const coord = coordinadoresData[index];

            if (coord) {
                modalImg.src = coord.foto;
                modalImg.alt = coord.nombre;
                modalName.textContent = coord.nombre;
                modalRole.textContent = coord.puesto;

                // Construir íconos sociales
                let socialsHTML = "";
                if (coord.socials.linkedin) {
                    socialsHTML += `
                        <a href="${coord.socials.linkedin}" target="_blank" rel="noopener noreferrer" class="coord-social-btn" aria-label="LinkedIn">
                            <svg viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                        </a>
                    `;
                }
                if (coord.socials.instagram) {
                    socialsHTML += `
                        <a href="${coord.socials.instagram}" target="_blank" rel="noopener noreferrer" class="coord-social-btn" aria-label="Instagram">
                            <svg viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
                        </a>
                    `;
                }

                modalSocials.innerHTML = socialsHTML;
                modal.classList.add("visible");
            }
        });

        // Cerrar modal
        modalClose.addEventListener("click", () => {
            modal.classList.remove("visible");
        });

        // Cerrar al clickear fuera del contenido del modal
        modal.addEventListener("click", (e) => {
            if (e.target === modal) {
                modal.classList.remove("visible");
            }
        });
    }
});
